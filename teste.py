...
def treinamento(A11, A12, A2, lista_treino):
    ...
    for epoca in range(10_000):
        ...
        for x1, x2, resultado_certo in lista_treino:
            ...
            A2.p1 = atualizar_peso_camada_saida(A2.p1, H, resultado_certo, saida_A11, taxa_aprendizagem)
            A2.p2 = atualizar_peso_camada_saida(A2.p2, H, resultado_certo, saida_A12, taxa_aprendizagem)
            A2.vies = atualizar_vies_camada_saida(A2.vies, H, resultado_certo, taxa_aprendizagem)

            # atualizar os pesos e viés de A11 (camada oculta)
            A11.p1 = atualizar_peso_camada_oculta(A11.p1, H, resultado_certo, A2.p1, saida_A11, x1, taxa_aprendizagem)
            A11.p2 = atualizar_peso_camada_oculta(A11.p2, H, resultado_certo, A2.p1, saida_A11, x2, taxa_aprendizagem)
            A11.vies = atualizar_vies_camada_oculta(A11.vies, H, resultado_certo, A2.p1, saida_A11, taxa_aprendizagem)

            # atualizar os pesos e viés de A12 (camada oculta)
            A12.p1 = atualizar_peso_camada_oculta(A12.p1, H, resultado_certo, A2.p2, saida_A12, x1, taxa_aprendizagem)
            A12.p2 = atualizar_peso_camada_oculta(A12.p2, H, resultado_certo, A2.p2, saida_A12, x2, taxa_aprendizagem)
            A12.vies = atualizar_vies_camada_oculta(A12.vies, H, resultado_certo, A2.p2, saida_A12, taxa_aprendizagem)

        if erro_total == 0:
            epocas_convergentes += 1  # contagem das épocas que convergiram

    # Porcentagem de convergência
    convergencia_percentual = (epocas_convergentes / 10_000) * 100
    print(f'Porcentagem de épocas que convergiram: {convergencia_percentual}%')
...
# Inicializa os neurônios com inicialização Xavier
...
treinamento(A11, A12, A2, treino_XOR)
